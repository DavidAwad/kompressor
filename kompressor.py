...
# An abstraction class to build up byte strings from bits.
...
#
# The return value is a 3-tuple (preamble, n, postamble). The deflated stream
# is contructed by writing the preamble, then n \x00 bytes, then the postamble.
...
def deflate_zeroes(n):
    ...
    # length of one, with one unused code." We store the unused code with code
    ...
    # write takes two bytes. We want byte operations for most of the data.
    # Push up to an even bit offset, but remember if we have to add an extra
    # bit later.
    #
...
"""
Function Definitions done, now let's import a secret image to take in,
and create our special PNG.
